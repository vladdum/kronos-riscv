...
#(
  ...
  // Stage 7a: gate all CSR side effects (register writes, priv transitions,
  // fetch-redirect pulses) on at_head_i.  Combinational paths (csr_new_val_o,
  // rdata_o) are deliberately left ungated so the retire trace sees the
  // correct post-write value even before the instruction reaches the ROB head.
  logic        csr_we_gated;
  ...
  // Stage 7a: csr_we_gated — the single master gate for all CSR side effects.
  // at_head_i is asserted only when the instruction is at the ROB head and
  // ready to commit; without it, CSR instructions execute speculatively but
  // their architectural state changes are suppressed.
  assign csr_we_gated     = req_i & at_head_i;

  // SW write to any FP CSR (fflags=0x001, frm=0x002, fcsr=0x003) marks FS=Dirty.
  assign fp_csr_sw_write  = csr_we_gated &
                           (addr_i == 12'h001
                          | addr_i == 12'h002
                          | addr_i == 12'h003);
  assign trig_csr_we_o    = csr_we_gated & trig_csr_match_i & (funct3_i[1:0] != 2'b00);
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (~mcountinhibit[0] & ~(csr_we_gated & (addr_i == 12'hB00))) mcycle <= mcycle + 64'd1;
      // minstret (gated by mcountinhibit[2] and qualified by retire; SW write takes priority)
      if (~mcountinhibit[2] & instret_retire_i & ~(csr_we_gated & (addr_i == 12'hB02)))
      ...
      for (int i = 3; i <= 10; i++) begin
        if (~mcountinhibit[i] & (mhpmevent[i] < 8'd32)
                              ...
                              & ~(csr_we_gated & (addr_i == 12'(12'hB00 + i))))
      ...
      else if (mret_i & at_head_i) begin
      ...
      else if (sret_i & at_head_i) begin
      ...
      // Gated by at_head_i: speculative CSR instructions are visible in
      // csr_new_val_o but do not commit to architectural state until at_head_i.
      if (csr_we_gated) begin
        unique case (addr_i)
