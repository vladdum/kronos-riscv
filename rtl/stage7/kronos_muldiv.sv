...
(
  ...
  assign valid_o    = (state_q == DONE) & ~complete_stall_i;
  assign idle_o     = (state_q == IDLE);
  assign result_o   = result_q;
  assign rob_idx_o  = rob_idx_q;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ...
        // ---------------------------------------------------------------
        DONE: begin
          if (!complete_stall_i) state_q <= IDLE;
        end
