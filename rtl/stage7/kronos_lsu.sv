...
// kronos_lsu.sv (stage7a) — thin adapter between the EX-stage pipeline
...
//
// Stage 7a additions:
//   at_head_i — gates dcache issue for stores/AMO/LR/SC; loads bypass the
//   gate so speculative loads can proceed before commit order is confirmed.
//
...
(
  ...
  // Stage 7a: ROB head indicator.  Stores, AMO, LR, and SC must not issue to
  // the dcache until the instruction is at the head of the reorder buffer
  // (i.e. it is guaranteed non-speculative).  Loads may issue speculatively.
  ...
  // Stage 7a: loads issue speculatively; stores/AMO/LR/SC require at_head_i.
  assign dcache_req_o     = req_i & ~pmp_fault_i & ~tlb_miss_i
                            & (~(we_i | is_amo_i | is_lr_i | is_sc_i) | at_head_i);
  ...
  assign dcache_amo_req_o = (is_lr_i | is_sc_i | is_amo_i)
                            & ~pmp_fault_i & ~tlb_miss_i & at_head_i;
