...
//
// rob_idx_i / rob_idx_o shadow tag_i / tag_o through lockstep shift-register
// pipes of equal depth inside this module.  The sub-units are unchanged.
//
...
//   FADD   7  (FADD, FSUB)
//   FMUL   9  (FMUL)
//   FMA    9  (FMADD, FMSUB, FNMADD, FNMSUB)
...
(
  ...
  // ---------------------------------------------------------------------------
  // rob_idx lockstep shift-register pipes (one per fixed-latency unit)
  // Depth matches each unit's pipeline latency exactly.
  // ---------------------------------------------------------------------------

  // FMISC: 1-cycle latency
  rob_idx_t fmisc_rob_idx_pipe [1];

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) fmisc_rob_idx_pipe <= '{default: '0};
    else         fmisc_rob_idx_pipe[0] <= rob_idx_i;
  end

  // FCVT: 3-cycle latency
  rob_idx_t fcvt_rob_idx_pipe [3];

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) fcvt_rob_idx_pipe <= '{default: '0};
    else begin
      fcvt_rob_idx_pipe[0] <= rob_idx_i;
      for (int i = 1; i < 3; i++) fcvt_rob_idx_pipe[i] <= fcvt_rob_idx_pipe[i-1];
    end
  end

  // FADD: 7-cycle latency
  rob_idx_t fadd_rob_idx_pipe [7];

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) fadd_rob_idx_pipe <= '{default: '0};
    else begin
      fadd_rob_idx_pipe[0] <= rob_idx_i;
      for (int i = 1; i < 7; i++) fadd_rob_idx_pipe[i] <= fadd_rob_idx_pipe[i-1];
    end
  end

  // FMUL: 9-cycle latency
  rob_idx_t fmul_rob_idx_pipe [9];

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) fmul_rob_idx_pipe <= '{default: '0};
    else begin
      fmul_rob_idx_pipe[0] <= rob_idx_i;
      for (int i = 1; i < 9; i++) fmul_rob_idx_pipe[i] <= fmul_rob_idx_pipe[i-1];
    end
  end

  // FMA: 9-cycle latency
  rob_idx_t fma_rob_idx_pipe [9];

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) fma_rob_idx_pipe <= '{default: '0};
    else begin
      fma_rob_idx_pipe[0] <= rob_idx_i;
      for (int i = 1; i < 9; i++) fma_rob_idx_pipe[i] <= fma_rob_idx_pipe[i-1];
    end
  end

  // ITER: variable latency — capture rob_idx_i when iter accepts a new request;
  // hold it in a register until iter's out_valid fires.
  rob_idx_t iter_rob_idx_q;

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) iter_rob_idx_q <= '0;
    else if (dispatch_ok & sel_iter & ~iter_busy)
      iter_rob_idx_q <= rob_idx_i;
  end
  ...
  // rob_idx_o is selected by the same priority as tag_o.
  // ---------------------------------------------------------------------------
  always_comb begin
    ...
    if (fmisc_out_valid) begin
      ...
      fflags_o  = fmisc_fflags;
      tag_o     = fmisc_tag;
      rob_idx_o = fmisc_rob_idx_pipe[0];
    end else if (fcvt_out_valid) begin
      result_o  = fcvt_result;
      fflags_o  = fcvt_fflags;
      tag_o     = fcvt_tag;
      rob_idx_o = fcvt_rob_idx_pipe[2];
    end else if (fadd_out_valid) begin
      result_o  = fadd_result;
      fflags_o  = fadd_fflags;
      tag_o     = fadd_tag;
      rob_idx_o = fadd_rob_idx_pipe[6];
    end else if (fmul_out_valid) begin
      result_o  = fmul_result;
      fflags_o  = fmul_fflags;
      tag_o     = fmul_tag;
      rob_idx_o = fmul_rob_idx_pipe[8];
    end else if (fma_out_valid) begin
      result_o  = fma_result;
      fflags_o  = fma_fflags;
      tag_o     = fma_tag;
      rob_idx_o = fma_rob_idx_pipe[8];
    end else if (iter_out_valid) begin
      result_o  = iter_result;
      fflags_o  = iter_fflags;
      tag_o     = iter_tag;
      rob_idx_o = iter_rob_idx_q;
    end
  end
