...
module tb_priv_csr;
  ...
  task automatic csr_write(input logic [11:0] a, input logic [63:0] v);
    @(posedge clk);
    addr_i     = a;
  ...
  task automatic drop_to_u;
    csr_write(12'h300, 64'h0000_0000_0000_0000);   // mstatus.MPP=00 (U)
    pulse_mret;
  endtask

  // Drop privilege to S via MPP=S + mret.
  task automatic drop_to_s;
    csr_write(12'h300, 64'h0000_0000_0000_0800);   // mstatus.MPP=01 (S)
    pulse_mret;
  endtask
  ...
  localparam logic [63:0] MEDELEG_MASK = 64'h0000_0000_0000_B7FF;
  ...
  initial begin
    ...
    // ----------------------------------------------------------------------
    csr_write(12'h100, 64'h0000_0000_0004_0000);       // sstatus: bit 18
    csr_read (12'h300, v, illegal);                    // mstatus
    if (v[18] !== 1'b1) begin
      $display("FAIL case2a: mstatus.SUM not set via sstatus write (mstatus=%h)", v);
      fail_count++;
    end
    csr_write(12'h100, 64'h0);                         // clear sstatus.SUM
    csr_read (12'h300, v, illegal);
    if (v[18] !== 1'b0) begin
    ...
    // ----------------------------------------------------------------------
    csr_write(12'h302, 64'hFFFF_FFFF_FFFF_FFFF);
    csr_read (12'h302, v, illegal);
    if (v[11] !== 1'b0) begin
    ...
    // ----------------------------------------------------------------------
    csr_write(12'h303, 64'hFFFF_FFFF_FFFF_FFFF);
    csr_read (12'h303, v, illegal);
    ...
    csr_write(12'h3A0, 64'h0000_0000_0000_0018);
    csr_read (12'h3A0, v, illegal);
    ...
    csr_write(12'h3A0, 64'h0000_0000_0000_0008);
    csr_read (12'h3A0, v, illegal);
    ...
    // ----------------------------------------------------------------------
    csr_write(12'h3A0, 64'h0000_0000_0000_0080);       // L=1
    csr_read (12'h3A0, v, illegal);
    if (v[7:0] !== 8'h80) begin
      $display("FAIL case6a: pmpcfg0[byte0] lock-set readback=%h expected 0x80", v[7:0]);
      fail_count++;
    end
    csr_write(12'h3A0, 64'h0000_0000_0000_0018);       // try NAPOT — locked!
    csr_read (12'h3A0, v, illegal);
    if (v[7:0] !== 8'h80) begin
    ...
    // ----------------------------------------------------------------------
    csr_write(12'h300, 64'h0000_0000_0000_0800);       // MPP = 01 (S)
    pulse_mret;
    ...
    csr_write(12'h100, 64'h0);                         // sstatus.SPP = 0
    pulse_sret;
    ...
    csr_read(12'hC00, v, illegal);                     // cycle (U-RO alias)
    ...
    csr_write(12'h306, 64'h0000_0000_0000_0001);       // mcounteren[0]=1
    csr_write(12'h106, 64'h0000_0000_0000_0001);       // scounteren[0]=1
    drop_to_u;
    csr_read(12'hC00, v, illegal);
    ...
    csr_read(12'h300, v, illegal);                     // mstatus (M-mode CSR)
    if (illegal !== 1'b1) begin
